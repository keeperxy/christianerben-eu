...
const ExperienceSection = () => {
  ...
  const experienceCategories = siteContent.experienceCategories;
                ...
                {exp.description.map((item, i) => (
                  <li key={i} className="flex items-start gap-2">
                    ...
                    <span className="flex-1">
                      {item.type === "achievement" ? (
                        <>
                          <span className="font-semibold text-primary">{t(siteContent.experienceAchievementPrefix)}</span>{" "}
                          {t(item.text)}
                        </>
                      ) : (
                        t(item.text)
          ...
          <div className="mt-16 space-y-14">
            <div>
              {experienceCategories && (
                <div className="mb-8 md:pl-12">
                  <h3 className="text-2xl md:text-3xl font-semibold">{t(experienceCategories.key.title)}</h3>
                  <p className="mt-2 text-sm md:text-base text-muted-foreground">{t(experienceCategories.key.subtitle)}</p>
                </div>
              )}
              {renderTimelineItems(groupedExperiences.key, "key")}
            </div>

            <div className="border-t border-border/60 pt-8">
              {experienceCategories && (
                <div className="mb-8 md:pl-12 opacity-80">
                  <h3 className="text-xl md:text-2xl font-semibold text-foreground/80">
                    {t(experienceCategories.additional.title)}
                  </h3>
              ...
              {renderTimelineItems(groupedExperiences.additional, "additional")}
            </div>
          </div>
